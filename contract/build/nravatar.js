...
function promiseResult$1(resultIdx) {
...
let Nravatar = (_dec = NearBindgen({}), _dec2 = view({}), _dec3 = view({}), _dec4 = view({}), _dec5 = call({
  ...
  get_avatars({
    ...
    for (let i = from_index; i < end; i++) {
      const account_id = this.avatars.keys.get(i);
      const avatar = this.get_avatar({
        account_id
      });
      ret.push(avatar);
    }
  ...
  set_avatar({
    ...
    let accountId = predecessorAccountId();
    let deposit = attachedDeposit();
    let oldAvatar = this.avatars.get(accountId);
    if (oldAvatar === null) {
      assert(deposit > STORAGE_COST, `Attach at least ${STORAGE_COST} yoctoNEAR`);
    }
    const promise = NearPromise.new(contract_id).functionCall("nft_token", bytes(JSON.stringify({
      token_id
    })), NO_DEPOSIT, FIVE_TGAS).then(NearPromise.new(currentAccountId()).functionCall("create_avatar", bytes(JSON.stringify({
...
function delete_avatar() {
  let _state = Nravatar._getState();
  if (!_state && Nravatar._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  let _contract = Nravatar._create();
  if (_state) {
    Nravatar._reconstruct(_contract, _state);
  }
  let _args = Nravatar._getArgs();
  let _result = _contract.delete_avatar(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Nravatar._serialize(_result));
}
function create_avatar() {
  ...
  let _args = Nravatar._getArgs();
  let _result = _contract.create_avatar(_args);
  Nravatar._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Nravatar._serialize(_result));
}
function set_avatar() {
  let _state = Nravatar._getState();
  ...
  let _args = Nravatar._getArgs();
  let _result = _contract.set_avatar(_args);
  Nravatar._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Nravatar._serialize(_result));
}
function get_avatars() {
  ...
  let _args = Nravatar._getArgs();
  let _result = _contract.get_avatars(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Nravatar._serialize(_result));
}
function number_of_avatar() {
  let _state = Nravatar._getState();
  if (!_state && Nravatar._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  let _args = Nravatar._getArgs();
  let _result = _contract.number_of_avatar(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Nravatar._serialize(_result));
}
function get_avatar() {
  let _state = Nravatar._getState();
  ...
  let _args = Nravatar._getArgs();
  let _result = _contract.get_avatar(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Nravatar._serialize(_result));
}
function promiseResult() {
  let success, owner_id;
  try {
    let result = promiseResult$1(0);
...
export { create_avatar, delete_avatar, get_avatar, get_avatars, number_of_avatar, set_avatar };
//# sourceMappingURL=nravatar.js.map
